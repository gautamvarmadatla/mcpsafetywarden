...
_DISAMBIGUATION_RULES: List[Tuple[re.Pattern, str, float, str]] = [
    (
        re.compile(
            r"(execute|run|eval)_(sql|query|queries|statement|select|insert|update|dml|ddl|sp|stored_proc)", re.I
        ),
        "mutating_write",
        0.88,
        "disambiguation_db_execution",
    ),
    (
        re.compile(r"(sql|query|statement|select|dml)_(execute|run|eval|exec)", re.I),
        "mutating_write",
        0.88,
        "disambiguation_db_execution",
    ),
    (
        re.compile(
            r"(run|execute|invoke)_(test|tests|spec|specs|suite|suites|check|checks|lint|benchmark|audit|scan|report)",
            re.I,
        ),
        "read_only",
        0.85,
        "disambiguation_test_execution",
    ),
    (
        re.compile(r"(call|invoke)_(local|function|method|procedure|proc|rpc_local|internal)", re.I),
        "mutating_write",
        0.78,
        "disambiguation_local_invocation",
    ),
    (
        re.compile(r"open_(file|document|doc|notebook|spreadsheet|csv|json|xml|pdf|log)", re.I),
        "read_only",
        0.82,
        "disambiguation_open_file_read",
    ),
    (
        re.compile(r"open_(connection|socket|session|browser|tab|url|link|stream_remote)", re.I),
        "external_action",
        0.82,
        "disambiguation_open_connection",
    ),
    (
        re.compile(r"(publish|release)_(file|report|artifact|result|results|output|local|doc|docs|asset|assets)", re.I),
        "additive_write",
        0.80,
        "disambiguation_publish_local",
    ),
    (
        re.compile(r"^process_(data|record|records|event|events|item|items|entry|entries|payload|message|batch)", re.I),
        "mutating_write",
        0.78,
        "disambiguation_process_data",
    ),
    (
        re.compile(r"(get|read|fetch)_(and_)?(send|post|email|notify|forward|relay|upload_to)", re.I),
        "external_action",
        0.85,
        "disambiguation_read_then_send",
    ),
    (
        re.compile(
            r"(execute|run|eval|exec)_(command|cmd|shell|bash|sh|zsh|script|arbitrary|code|binary|program)", re.I
        ),
        "external_action",
        0.92,
        "disambiguation_shell_execution",
    ),
    (
        re.compile(r"^log_(event|entry|entries|metric|metrics|trace|error|warning|audit_event)", re.I),
        "additive_write",
        0.78,
        "disambiguation_log_write",
    ),
    (
        re.compile(r"format_(disk|drive|partition|volume|storage)", re.I),
        "destructive",
        0.92,
        "disambiguation_format_disk",
    ),
    (
        re.compile(r"(stop|kill|terminate|shutdown)_(server|service|process|container|instance|pod|vm|node)", re.I),
        "destructive",
        0.85,
        "disambiguation_stop_service",
    ),
    (
        re.compile(r"(invoke|call)_(lambda|function_arn|api|endpoint|webhook|remote|external|http|url|service)", re.I),
        "external_action",
        0.88,
        "disambiguation_remote_invocation",
    ),
    (
        re.compile(r"(post|publish|push)_(to_|message_to|event_to|data_to|update_to)", re.I),
        "external_action",
        0.85,
        "disambiguation_post_to_external",
    ),
    (
        re.compile(r"(copy|move|rename)_(file|files|dir|directory|folder|path|item)", re.I),
        "mutating_write",
        0.80,
        "disambiguation_local_file_op",
    ),
    (
        re.compile(r"deploy_(config|configuration|setting|settings|policy|policies|rule|rules)", re.I),
        "mutating_write",
        0.75,
        "disambiguation_deploy_config",
    ),
    (
        re.compile(r"(bulk|batch|mass|cascade)_(delete|remove|purge|erase|wipe|destroy)", re.I),
        "destructive",
        0.90,
        "disambiguation_bulk_delete",
    ),
]

_EFFECT_PATTERNS_COMPILED: List[Tuple[str, List[re.Pattern]]] = [
    (effect, [re.compile(p, re.IGNORECASE) for p in patterns]) for effect, patterns in EFFECT_PATTERNS
]

_EXEC_PREFIX_RE = re.compile(r"^(sudo|root|elevated|admin|privileged)_", re.IGNORECASE)
_DESTRUCTIVE_PREFIX_RE = re.compile(
    r"^(kill|nuke|wipe|purge|drop|destroy|terminate|erase|delete|remove)_", re.IGNORECASE
)
...
_DESC_EFFECT_SIGNALS: List[Tuple[re.Pattern, str, float]] = [
    (
        re.compile(
            r"\b(read[s]?|retriev|fetch|list[s]?|search|query|queries|look.?up|find[s]?|get[s]?|show[s]?|view[s]?|describe[s]?|inspect[s]?|check[s]?|monitor[s]?|watch[es]?|scan[s]?|report[s]?|stat[s]?|count[s]?|summarize[s]?|display[s]?|return[s]?|output[s]?|enumerate[s]?|browse[s]?|crawl[s]?|scrape[s]?|capture[s]?|sample[s]?|introspect[s]?|poll[s]?|observe[s]?)\b",
            re.IGNORECASE,
        ),
        "read_only",
        0.45,
    ...
    (
        re.compile(
            r"\b(creat|add[s]?|insert[s]?|append[s]?|upload[s]?|generat|build[s]?|spawn[s]?|provision[s]?|register[s]?|save[s]?|store[s]?|record[s]?|log[s]?|track[s]?|open[s]? (a |an )?(ticket|issue|pr|pull request|bug))\b",
            re.IGNORECASE,
        ),
        "additive_write",
        0.45,
    ...
    (
        re.compile(
            r"\b(updat|edit[s]?|modif|patch[es]?|replac|renam|mov[es]?|configur|toggl|assign[s]?|sync[s]?|migrat|restor|rollback|revert|reset[s]?|clear[s]?|flush[es]?|purge[s]?|normaliz|enriche[s]?|redact[s]?|mask[s]?|process[es]?)\b",
            re.IGNORECASE,
        ),
        "mutating_write",
        0.45,
    ...
    (
        re.compile(
            r"\b(delet|remov|drop[s]?|purg[es]?|eras[es]?|destroy[s]?|terminat|kill[s]?|wip[es]?|unlink[s]?|shutdown[s]?|expir[es]?|invalidat|decommission[s]?|deregister[s]?|factory.?reset)\b",
            re.IGNORECASE,
        ),
        "destructive",
        0.50,
    ...
    (
        re.compile(
            r"\b(send[s]?|email[s]?|notif|messag|sms|webhook[s]?|broadcast[s]?|dispatch[es]?|trigger[s]?|deploy[s]?|provision[s]? (a |an )?(server|instance|resource|cloud)|pay[s]?|charg[es]?|alert[s]?|invoke[s]? (a |an )?(lambda|function|api|endpoint|remote)|ssh[es]?|execut[es]? (a |an )?(shell|command|script|binary))\b",
            re.IGNORECASE,
        ),
        "external_action",
        0.50,
...
_SCHEMA_SIGNALS: List[Tuple[re.Pattern, str, float, str]] = [
    (
        re.compile(
            r"^(recipient|to|email|phone|mobile|sms_to|channel|topic|webhook_url|slack_channel|teams_channel)$",
            re.IGNORECASE,
        ),
        "external_action",
        0.82,
        "schema_has_recipient_field",
    ),
    (
        re.compile(r"^(subject|body|content|message|text|payload|html|template|attachment)$", re.IGNORECASE),
        "external_action",
        0.55,
        "schema_has_message_body_field",
    ),
    (
        re.compile(
            r"^(command|cmd|shell|bash|script|code|eval|expression|statement|program|binary|executable|args|arguments|argv)$",
            re.IGNORECASE,
        ),
        "external_action",
        0.88,
        "schema_has_execution_field",
    ),
    (
        re.compile(
            r"^(password|passwd|secret|token|api_key|apikey|access_key|private_key|pem|cert|certificate|passphrase|credential|auth_token|bearer|signing_key|encryption_key|ssh_key)$",
            re.IGNORECASE,
        ),
        "read_only",
        0.60,
        "schema_has_credential_field",
    ),
    (
        re.compile(
            r"^(path|file_path|filepath|filename|file|directory|dir|dest|destination|target_path|output_path)$",
            re.IGNORECASE,
        ),
        "additive_write",
        0.55,
        "schema_has_file_path",
    ),
    (
        re.compile(r"^(overwrite|force|replace|truncate|clobber|mode|flags)$", re.IGNORECASE),
        "mutating_write",
        0.70,
        "schema_has_overwrite_flag",
    ),
    (
        re.compile(r"^(table|collection|index|database|db|keyspace|namespace|schema)$", re.IGNORECASE),
        "mutating_write",
        0.58,
        "schema_has_db_target",
    ),
    (
        re.compile(
            r"^(cascade|recursive|hard_delete|permanent|purge_all|wipe|drop_table|force_delete)$", re.IGNORECASE
        ),
        "destructive",
        0.85,
        "schema_has_destructive_flag",
    ),
    (
        re.compile(
            r"^(host|hostname|ip|ip_address|port|endpoint|url|base_url|server|remote|ssh_host|rdp_host)$", re.IGNORECASE
        ),
        "external_action",
        0.72,
        "schema_has_network_target",
    ),
    (
        re.compile(r"^(role|permission|policy|acl|scope|grant|privilege|access_level|user_role|group)$", re.IGNORECASE),
        "mutating_write",
        0.68,
        "schema_has_permission_field",
    ),
    (
        re.compile(r"^(query|filter|search|q|keyword|term|expression|pattern|regex|selector|xpath|jq)$", re.IGNORECASE),
        "read_only",
        0.65,
        "schema_has_query_field",
    ),
    (
        re.compile(r"^(page|limit|offset|cursor|per_page|page_size|max_results|count|skip|from|size)$", re.IGNORECASE),
        "read_only",
        0.60,
        "schema_has_pagination_field",
    ),
    (
        re.compile(
            r"^(cron|schedule|interval|delay|at|run_at|trigger_at|timeout|ttl|retry_count|retry_policy)$", re.IGNORECASE
        ),
        "external_action",
        0.62,
        "schema_has_scheduling_field",
    ),
    (
        re.compile(
            r"^(amount|price|cost|charge|fee|currency|payment_method|card|stripe_token|billing)$", re.IGNORECASE
        ),
        "external_action",
        0.80,
        "schema_has_financial_field",
    ),
]

_OUTPUT_RISK_SIGNALS: List[Tuple[re.Pattern, str, str]] = [
    (
        re.compile(r"\b(binary|blob|bytes|raw|base64|file_content|attachment|image|video|audio|stream)\b", re.I),
        "high",
        "output_may_be_binary_blob",
    ),
    (
        re.compile(r"\b(password|secret|token|api.?key|credential|private.?key|pem|cert|ssh)\b", re.I),
        "high",
        "output_may_contain_credentials",
    ),
    (
        re.compile(r"\b(env|environment|environ|process\.env|os\.environ|dotenv|\.env)\b", re.I),
        "high",
        "output_may_expose_env_vars",
    ),
    (
        re.compile(r"\b(list|array|items|results|rows|records|entries|nodes|edges|all_)\b", re.I),
        "medium",
        "output_is_list_or_collection",
    ),
    (
        re.compile(r"\b(file|directory|path|filesystem|disk|storage)\b", re.I),
        "medium",
        "output_contains_filesystem_data",
    ),
    (
        re.compile(r"\b(log[s]?|trace|debug|stack.?trace|error.?detail|diagnostic)\b", re.I),
        "medium",
...
def _classify_by_name(tool_name: str) -> Tuple[str, float, List[str]]:
    ...
    for effect_class, patterns in _EFFECT_PATTERNS_COMPILED:
        for rx in patterns:
            if rx.search(lower):
                return effect_class, 0.65, [f"name_matches_{effect_class}_pattern"]
...
def _classify_by_description(description: str) -> Tuple[str, float, List[str]]:
    ...
    if not votes:
        return "unknown", 0.0, ["no_description_signals"]
...
def _classify_by_schema(schema: Dict[str, Any]) -> Tuple[str, float, List[str]]:
    ...
    if not props:
        return "unknown", 0.30, ["no_schema_properties"]

    param_names = {k.lower() for k in props}
    hints: List[Tuple[str, float, str]] = []

    for rx, effect, conf, label in _SCHEMA_SIGNALS:
        if any(rx.match(p) for p in param_names):
            hints.append((effect, conf, label))
    ...
    if body_fields & param_names and recv_fields & param_names:
        hints.append(("external_action", 0.90, "schema_body_plus_recipient_compound"))

    exec_fields = {"command", "cmd", "shell", "script", "code", "eval", "executable"}
    if exec_fields & param_names:
        hints.append(("external_action", 0.92, "schema_execution_compound"))

    if {"path", "file_path", "filepath", "filename"} & param_names:
        if {"overwrite", "force", "truncate", "replace", "clobber"} & param_names:
            hints.append(("destructive", 0.78, "schema_path_plus_overwrite_compound"))
    ...
    if test_fields & param_names:
        hints.append(("read_only", 0.78, "schema_has_test_identifier"))
    ...
    if {"amount", "price", "cost"} & param_names and {
        "currency",
        "payment_method",
        "card",
        "stripe_token",
    ...
    if not hints:
        return "unknown", 0.30, ["no_schema_pattern_matched"]
...
def _infer_output_risk(schema: Dict[str, Any], description: str, effect_class: str) -> Tuple[str, List[str]]:
    ...
    if effect_class == "read_only":
        return "low", ["read_only_low_output_risk"]
...
def _infer_open_world(description: str, schema: Dict[str, Any], effect_class: str) -> Tuple[bool, List[str]]:
    ...
    if _DESC_OPEN_WORLD_SIGNALS.search(combined):
        evidence.append("open_world_external_signal_in_description_or_schema")
    ...
    if network_params & schema_props:
        evidence.append("schema_has_network_target_param")

    if effect_class == "external_action":
        evidence.append("effect_class_is_external_action")
...
def _infer_extra_risks(description: str, schema: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if _DESC_CREDENTIAL_SIGNALS.search(combined):
        risks["credential_access_risk"] = True

    if _DESC_EXEC_SIGNALS.search(combined):
        risks["arbitrary_execution_risk"] = True

    if _DESC_FILESYSTEM_SIGNALS.search(combined):
        risks["filesystem_access_risk"] = True
...
) -> Tuple[Optional[str], float, Optional[bool], Optional[bool], List[str]]:
...
def _classify_with_llm(
    ...
    raw_conf = {k: max(0.0, min(1.0, float(v))) if isinstance(v, (int, float)) else 0.0 for k, v in raw_conf.items()}
    ...
    for extra in (
        ...
        "arbitrary_execution_risk",
        "filesystem_access_risk",
    ...
    risk_tags = [
        t
        for t in (
            parsed.get("risk_tags")
            if isinstance(parsed.get("risk_tags"), list)
            else ([parsed["risk_tags"]] if isinstance(parsed.get("risk_tags"), str) else [])
        )
        if t in _VALID_RISK_TAGS
    ]
...
def classify_tool(
    ...
    if llm_provider:
        try:
            ...
            if raw_llm:
                ...
                if raw_llm.get("effect_class") not in (None, "unknown"):
                    llm_result = raw_llm
    ...
    for eff, conf in [
        ...
        if eff and eff != "unknown":
            votes[eff] = votes.get(eff, 0.0) + conf
    ...
    if not votes:
        final_effect, final_conf = "unknown", 0.25
    else:
    ...
    if retry_hint is True:
        retry_safety = "safe"
    elif retry_hint is False:
        retry_safety = "unsafe"
    elif final_effect in RETRY_SAFE_EFFECTS:
        retry_safety = "safe"
    elif final_effect in RETRY_UNSAFE_EFFECTS:
        retry_safety = "unsafe"
    else:
        retry_safety = "caution"
    ...
    if _saved_sec_finding:
        result["_security_finding"] = _saved_sec_finding
    return result

...
from ..scan.scanner import (
    ...
    run_cisco_scan,
    run_snyk_scan,
    auto_detect_providers as _auto_detect_providers,
...
async def security_scan_server(
    ...
    _log.info(
        "security_scan_server server_id=%s providers=%s timeout=%ds background=%s",
        server_id,
        providers_to_run,
        scan_timeout_s,
        background,
...
async def scan_all_servers(
    ...
    if not all_server_rows:
        return json.dumps(
            {"error": "No registered servers found.", "hint": "Run register_server or onboard_server first."}
        )

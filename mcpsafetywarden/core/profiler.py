...
def compute_profile_from_runs(tool_id: str, prior: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if total >= 5:
        ...
        profile["confidence"] = {k: round(min(0.97, v + boost), 2) for k, v in prior.get("confidence", {}).items()}
        extra_evidence = [
            f"observed_{total}_runs",
            f"failure_rate={profile['failure_rate']}",
        ]
        if profile.get("schema_stability") is not None:
            extra_evidence.append(f"schema_stability={profile['schema_stability']}")
...
def get_or_build_profile(
    ...
    if fresh_prior is None and stored is not None:
        return stored

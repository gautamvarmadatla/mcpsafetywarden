...
Claude sees only the wrapper's 23 tools. To use github or slack it must call `safe_tool_call(server_id="github", ...)` - registration is enforced because `safe_tool_call` rejects any unregistered `server_id`.
